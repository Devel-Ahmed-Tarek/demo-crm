...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-[#1b1b18] dark:text-[#EDEDEC]">{{ __('Create Apartment') }}</h1>
    ...
    <form method="POST" action="{{ route('units.apartments.store', $unit) }}" class="bg-white dark:bg-[#161615] rounded-lg shadow-sm border border-[#e3e3e0] dark:border-[#3E3E3A] p-6 space-y-6">
        @csrf

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label class="block text-sm font-medium text-[#1b1b18] dark:text-[#EDEDEC] mb-2">{{ __('Floor') }} *</label>
                <input type="number" name="floor" value="{{ old('floor', request('floor')) }}" required min="0" placeholder="e.g., 0 for ground floor"
            ...
            <div>
                ...
                <input type="number" name="column" value="{{ old('column', request('column')) }}" required min="1" placeholder="e.g., 1, 2, 3..."
                    class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-lg bg-white dark:bg-[#0a0a0a] text-[#1b1b18] dark:text-[#EDEDEC]">
                @error('column')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="text" name="code" value="{{ old('code') }}" required placeholder="e.g., A-001"
                    class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-lg bg-white dark:bg-[#0a0a0a] text-[#1b1b18] dark:text-[#EDEDEC]">
                @error('code')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select name="status" required
                    ...
                    <option value="owner" {{ old('status') == 'owner' ? 'selected' : '' }}>{{ __('Not for sale') }}</option>
                </select>
                @error('status')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" name="area" value="{{ old('area') }}" min="0" step="0.01" placeholder="e.g., 120.5"
                    class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-lg bg-white dark:bg-[#0a0a0a] text-[#1b1b18] dark:text-[#EDEDEC]">
                @error('area')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" name="rooms" value="{{ old('rooms') }}" min="0" placeholder="e.g., 3"
                    class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-lg bg-white dark:bg-[#0a0a0a] text-[#1b1b18] dark:text-[#EDEDEC]">
                @error('rooms')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" name="price" value="{{ old('price') }}" min="0" step="0.01" placeholder="e.g., 150000"
                    class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-lg bg-white dark:bg-[#0a0a0a] text-[#1b1b18] dark:text-[#EDEDEC]">
                @error('price')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
