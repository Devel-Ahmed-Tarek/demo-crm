...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-[#1b1b18] dark:text-[#EDEDEC]">{{ __('Add Testimonial') }}</h1>
            <p class="text-sm text-[#706f6c] dark:text-[#A1A09A] mt-1">{{ __('Add a new client testimonial') }}</p>
        </div>
    ...
    <div class="bg-white dark:bg-[#161615] rounded-xl shadow-lg border border-[#e3e3e0] dark:border-[#3E3E3A] p-6">
        <form action="{{ route('site.testimonials.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea id="testimonial" name="testimonial" rows="4" required
                        class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-lg bg-white dark:bg-[#0a0a0a] text-[#1b1b18] dark:text-[#EDEDEC]">{{ old('testimonial') }}</textarea>
                </div>
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="rating" name="rating"
                            class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-lg bg-white dark:bg-[#0a0a0a] text-[#1b1b18] dark:text-[#EDEDEC]">
                            <option value="5" {{ old('rating', 5) == 5 ? 'selected' : '' }}>5 {{ __('Stars') }}</option>
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" id="property_sold" name="property_sold" value="{{ old('property_sold') }}"
                            class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-lg bg-white dark:bg-[#0a0a0a] text-[#1b1b18] dark:text-[#EDEDEC]"
