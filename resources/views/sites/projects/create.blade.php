...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-[#1b1b18] dark:text-[#EDEDEC]">{{ __('Add Project to') }}: {{ $site->name }}</h1>
        <a href="{{ route('sites.show', $site) }}" class="text-[#706f6c] dark:text-[#A1A09A] hover:text-[#1b1b18] dark:hover:text-[#EDEDEC]">
            {{ __('Back') }}
        </a>
    </div>

    <form method="POST" action="{{ route('sites.projects.store', $site) }}" enctype="multipart/form-data" class="bg-white dark:bg-[#161615] rounded-lg shadow-sm border border-[#e3e3e0] dark:border-[#3E3E3A] p-6 space-y-6">
        @csrf
        ...
        <div>
            ...
            <select name="type" required
                class="w-full px-4 py-2 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-lg bg-white dark:bg-[#0a0a0a] text-[#1b1b18] dark:text-[#EDEDEC]">
                <option value="current" {{ old('type') == 'current' ? 'selected' : '' }}>{{ __('Current Project') }}</option>
                <option value="previous" {{ old('type') == 'previous' ? 'selected' : '' }}>{{ __('Previous Project') }}</option>
            </select>
            @error('type')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
